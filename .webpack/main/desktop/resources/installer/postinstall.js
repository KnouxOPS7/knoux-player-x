...
function registerFileAssociations() {
    if (process.platform === 'win32') {
        ...
        for (const ext of fileTypes) {
            try {
                child_process.execSync(`reg add "HKCR\\.${ext}\\OpenWithProgids" /ve /d "KNOUX.PlayerX" /f`, { stdio: 'ignore' });
...
function initializeUserData() {
    const userDataPath = path.join(process.platform === 'win32'
        ? process.env.LOCALAPPDATA
        : process.platform === 'darwin'
            ? path.join(os.homedir(), 'Library', 'Application Support')
            : path.join(os.homedir(), '.config'), 'KNOUX_Player_X');
    try {
